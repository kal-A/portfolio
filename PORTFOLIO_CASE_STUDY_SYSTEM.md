...
2. **Subheading attachment / semantic attachment.** Instructional labels, subsection headings, captions, and prompts must sit visually close to the component they introduce (small gap, not a section-sized gap) — they should never be separated from it by section-scale whitespace. Where a section has a primary boxed/pill-styled eyebrow (e.g. "WHAT CHANGED") and a secondary subgroup within the same section (e.g. "EARLIER CONTRIBUTIONS"), give the secondary subheading the *same* boxed/pill treatment so both read as one shared visual grammar, and keep the gap above it a deliberate subgroup gap, not a section-sized one.
...
6. **Flow + panel alignment.** The detail panel itself stays top-aligned within its cell (not vertically centered) so it reads consistently regardless of which step is active. On the flow side, when the flow's rows are meaningfully shorter than the fixed-height detail panel beside them, vertically center the row stack (not top-align it) within its column, so the whole assembly balances against the panel instead of leaving dead space beneath the shorter side. Caveat: centering a real two-row stack (`justify-center`) only relocates the dead space — it removes it from below but reintroduces the same amount above, which reads as a broken gap if an instructional label sits directly above the flow. When there are two genuine rows AND a label sits above them, anchor row one to the top and row two to the bottom instead (`justify-between`) so the label stays attached (rule 2) while the assembly still fills the panel's full height with no dead space at either end. Reserve true centering for the single-row case (no second row to anchor to the bottom).
