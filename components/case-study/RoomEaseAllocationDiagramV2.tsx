 ...
 * the dark token set, and the click-to-inspect interaction changes from a
 * full-height fixed white drawer (which broke out of the page and dwarfed the
 * content) to a compact detail panel docked in the column beside the diagram,
 * using the horizontal space the tall portrait chart would otherwise waste.
 *
 * Shape still carries meaning: pill = event / end state, rectangle = process,
 * diamond = decision (>=2 labelled branches), document = record. Colours are
 * driven entirely by the case-study tokens (surface-2 nodes, gold decisions,
 * muted connectors) so the chart reads as part of the page instead of a
 * pasted-in artifact.
 */
...
export default function RoomEaseAllocationDiagramV2() {
  ...
  return (
    <div className="grid w-full lg:grid-cols-[minmax(0,1fr)_300px] gap-6 items-start">
      {/* Diagram card */}
      <div
        className="re2-diagram rounded-[var(--radius-default)] overflow-auto border min-w-0"
        ...
        {activeNode ? (
          ...
          >
            ...
            <p className="mt-3 text-sm" style={{ color: "var(--color-text-muted)", lineHeight: "var(--leading-body)" }}>
              Click any node to see what that step involves. Shape carries meaning, not just position:
            </p>
            <ul className="mt-4 flex flex-col gap-2.5">
              {(Object.keys(TYPE_LABEL) as NodeType[]).map((t) => (
                <li key={t} className="flex items-center gap-2.5 text-sm" style={{ color: "var(--color-text-muted)" }}>
                  <LegendSwatch type={t} />
                  {TYPE_LABEL[t]}
                </li>
              ))}
